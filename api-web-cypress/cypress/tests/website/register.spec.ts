...
describe("register scenario", () => {
  ...
  it("register with email address", () => {
    ...
    cy.get("button").contains("Daftar").should("be.visible").click();
    ...
    cy.get(".bl-flex-item")
      ...
      .should("be.visible");
    cy.get("button").contains("Bukan, ganti email").should("be.visible");
    cy.get("button").contains("Ya, kirim kode").should("be.visible").click();
    ...
    cy.get(".bl-flex-item").children("p").contains(email).should("be.visible");
    cy.get("label").contains("Kode rahasia").should("be.visible");
    cy.get("button").should("be.disabled");
  });

  it("register with phone number", () => {
    const phone_number =
      "083" + faker.random.number({ min: 1000000, max: 9999999999 }).toString();
    ...
    cy.get("button").contains("Bukan, ganti nomor").should("be.visible");
    cy.get("button").contains("Ya, kirim kode").should("be.visible").click();
  ...
  it("register with registered email", () => {
    cy.fixture("users").then((users) => {
      ...
      cy.get(".mb-8.bl-button")
        ...
        .and("be.visible");
      cy.get("button").contains("Ganti email").should("be.visible");
    });
  });
  ...
  it("register with registered phone number", () => {
    cy.fixture("users").then((users) => {
      ...
      cy.get(".mb-8.bl-button")
        ...
        .and("be.visible");
      cy.get("button").contains("Ganti no. hp").should("be.visible");
    });
  });
});
